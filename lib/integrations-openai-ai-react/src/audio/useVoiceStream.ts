...
interface StreamCallbacks {
  ...
  /**
   * Any event on the stream this hook does not own. The transport handles
   * transcripts, audio and completion; an application that multiplexes its own
   * events onto the same stream (who is speaking, for instance) receives them
   * here rather than having them rejected as malformed.
   */
...
type VoiceStreamEvent = TypedVoiceStreamEvent | DoneEvent | AppEvent;
...
function isVoiceStreamEvent(value: unknown): value is VoiceStreamEvent {
  ...
  switch (record.type) {
    ...
    default:
      // Not a transport event, but still a well-formed one: pass it to the
      // application instead of failing the stream.
...
): event is TypedVoiceStreamEvent {
  return (
    "type" in event && TRANSPORT_EVENT_TYPES.has((event as AppEvent).type)
  );
}
...
function handleVoiceStreamEvent(
  ...
  if (!isTransportEvent(event)) {
    callbacks.onEvent?.(event as AppEvent);
    return;
  }
...
export function useVoiceStream({ workletPath, ...callbacks }: StreamCallbacks) {
  ...
  /**
   * Cuts the current response off immediately: stops the network read and
   * drops whatever audio is still queued in the worklet. Used for barge-in,
   * where continuing to play over someone who has started speaking is the
   * whole problem.
   */
